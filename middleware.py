...
class RateLimitMiddleware(BaseHTTPMiddleware):
    """Middleware basique pour le rate limiting (? am?liorer avec Redis)"""
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        if self.request_counts[client_ip]["count"] >= self.requests_per_minute:
            logger.warning(f"Rate limit d?pass? pour {client_ip}")
            return Response(
...
class AuthMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        # Ignorer les endpoints de sant? et de documentation
        if request.url.path in ["/health", "/docs", "/redoc", "/openapi.json"]:
            return await call_next(request)
        
        # Si aucune API key n'est configur?e, passer
        ...
        if token != settings.api_key:
            logger.warning(f"Tentative d'acc?s avec un token invalide depuis {request.client.host}")
            raise HTTPException(
                status_code=403,
